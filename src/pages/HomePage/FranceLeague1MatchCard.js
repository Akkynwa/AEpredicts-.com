...
const API = "http://localhost:5000/franceleague1";
    ...
    fetch(API)
      ...
      .then((data) => {
        if (Array.isArray(data)) {
          setMatches(data);
        } else if (Array.isArray(data.franceleague1)) {
          setMatches(data.franceleague1);
        } else {
          setMatches([]);
        }
        setLoading(false);
    ...
    <Container fluid className="my-4">
      {loading ? (
        ...
        >
          ...
          <Card.Header
            style={{
              background: "linear-gradient(180deg, #0055A4, #002654)",              
              color: "white",
              fontWeight: "bold",
              display: "flex",
...
export default FranceLeague1MatchCard;