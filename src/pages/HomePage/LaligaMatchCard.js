// src/components/LaligaMatchCard.jsx
    ...
    fetch(API)
      ...
      .then((data) => {
        if (Array.isArray(data)) {
          setMatches(data);
        } else if (Array.isArray(data.laliga)) {
          setMatches(data.laliga);
        } else {
          setMatches([]);
        }
        setLoading(false);
    ...
    <Container fluid className="my-4">
      {loading ? (
        ...
        >
          ...
          <Card.Header
            style={{
              background: "linear-gradient(180deg, #800000,#2c0909)",              
              color: "white",
              fontWeight: "bold",
              display: "flex",
...
export default LaligaMatchCard;
